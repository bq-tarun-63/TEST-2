...
interface RestoreBlockRequestBody {
    blockId: string;
    workspaceId: string;
}
...
export async function POST(req: NextRequest) {
    try {
        ...
        const body = (await req.json()) as RestoreBlockRequestBody;
        ...
        await BlockService.restoreFromTrash({
            userId: String(auth.user.id),
            blockId,
            workspaceId,
        });

        return NextResponse.json({ success: true, message: "Block restored from trash" }, { status: 200 });
    } catch (error: any) {
        console.error("Error in /api/note/block/restore:", error);
