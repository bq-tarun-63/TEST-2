...
export interface MathematicsOptions {
  ...
  // biome-ignore lint/suspicious/noExplicitAny: HTML attributes can be arbitrary
...
export const Mathematics = Node.create<MathematicsOptions>({
  ...
  addNodeView() {
    return ({ node, HTMLAttributes, getPos, editor }: any) => {
      ...
      dom.innerHTML = katex.renderToString(String((node as any).attrs?.latex ?? ""), this.options.katexOptions);
